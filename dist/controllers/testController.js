...
exports.recordQuestionStat = recordQuestionStat;
exports.recordUserAnswer = recordUserAnswer;
...
async function startTest(req, res) {
    try {
        ...
        for (const cat of categoriesConfig) {
            ...
            const normalPick = await prisma.$queryRawUnsafe(`
          SELECT *
          FROM "Question"
        ...
        `);
            ...
            const possibleBad = await prisma.$queryRawUnsafe(`
          SELECT q.*,
        ...
        `);
            ...
            if (howManyToInject > 0) {
                ...
                for (const badObj of pickedBad) {
                    ...
                    if (normalPick.some((x) => x.id === bq.id))
                        continue;
...
async function finishTest(req, res) {
    try {
        ...
        if (updateAnswers.length > 0) {
            await prisma.$transaction(updateAnswers.map((ans) => prisma.userAnswer.update({
                where: {
...
async function recordQuestionStat(req, res) {
    try {
        const userId = req.user?.id;
...
async function recordUserAnswer(req, res) {
    try {
        const userId = req.user?.id;
